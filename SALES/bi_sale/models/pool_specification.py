...
class PoolSpecificationLineSale(models.Model):
    _name = "pool.specification.line.sale"
    _description = "pool specification line sale"

    sale_pool_specification_id = fields.Many2one(string='Pool Specification ID',comodel_name='sale.order')
    ...
    @api.depends('area_of_pool', 'volume_of_water', 'sale_pool_specification_id.pool_dimension_ids')
    def _compute_pool_specification(self):
        for record in self:
            record.area_of_pool = 0.0
            record.volume_of_water = 0.0

            if record.sale_pool_specification_id:  
                for line in record.sale_pool_specification_id.pool_dimension_ids:
                    if line.pool_dimension_sl_no == record.pool_specification_sl_no:
                        record.volume_of_water = line.water_volume if line.water_volume else 0.0
                        record.area_of_pool = line.total_surface_area if line.total_surface_area else 0.0
