...
class SaleOrder(models.Model):
    ...
    discount_rate = fields.Float('Discount Rate', digits=(16, 3),
    ...
    analytic_account_id = fields.Many2one('account.analytic.account',string="Custom Analytic Account",)
    ...
    def _onchange_type(self):
        amc_analytic = self.env['account.analytic.account'].search([('is_amc', '=', True)],limit=1)
        vas_analytic = self.env['account.analytic.account'].search([('is_vas', '=', True)],limit=1)
        if self.type == 'amc':
            self.analytic_account_id = amc_analytic.id
        if self.type == 'vas':
            self.analytic_account_id = vas_analytic.id
    ...
    #     for order in self:
    #         if order.discount_type == 'percent' and any(not line.discount_amount for line in order.order_line):
    #             for line in order.order_line:
    #                 line.discount = order.discount_rate
    #         elif order.discount_type == 'amount' and any(not line.discount_amount for line in order.order_line):
    #             total = 0.0
    #             for line in order.order_line:
    ...
    def action_analytic_account(self):
        analytic_map = {
        'amc': self.env['account.analytic.account'].search([('is_amc', '=', True)], limit=1),
        'vas': self.env['account.analytic.account'].search([('is_vas', '=', True)], limit=1)
        }
        sale_orders = self.env['sale.order'].search([('type', 'in', ['amc', 'vas'])])
        for rec in sale_orders:
            analytic = analytic_map.get(rec.type)
            if analytic:
