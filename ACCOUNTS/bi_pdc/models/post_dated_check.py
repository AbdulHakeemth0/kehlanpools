...
class PostDatedCheck(models.Model):
    ...
    #     if self.pdc_type == "vendor":
    #         vendor_journal = self.env.company.issued_journal_id
    #         if not vendor_journal:
    #             raise UserError(_("Please configure the PDC Issued Journal"))
    #         self.journal_id = vendor_journal
    #     elif self.pdc_type == "customer":
    #         customer_journal = self.env.company.received_journal_id
    #         if not customer_journal:
    #             raise UserError(_("Please configure the PDC Received Journal"))
    #         self.journal_id = customer_journal
    ...
    # @api.depends('payment_line_ids.amountpaid')
    # def _compute_check_amount(self):
    #     check_amount = 0.0
    #     for record in self:
    #         for line in record.payment_line_ids:
    #             check_amount += line.amountpaid
    #         record.update({"check_amount": check_amount})
    ...
    customer_domain = fields.Binary(compute="_compute_customer_domain")
    ...
    def _prepare_default_reversal_bounce(self, move):
        return {
            ...
            "auto_post": fields.Date.context_today(self),
    ...
    # def action_customer_and_vendor_pdc_notification(self):
    #     users = self.env["res.users"].search(
    #         [("groups_id", "in", self.env.ref("bi_collection_report.financial_accounts_group").id)]
    #     )
    #     today = fields.Date.today()
    #     yesturday_date = today - timedelta(days=1)
    #     for user in users:
    #         records = self.env["post.dated.check"].search([("state", "not in", ["draft", "cancel"])])
    #         for rec in records:
    #             if rec.date == yesturday_date:
    #                 data = {
    #                     "res_id": rec.id,
    #                     "res_model_id": 783,
    #                     "user_id": user.id,
    #                     "summary": "PDC " + str(rec.name) + " is going to expire on " + str(rec.date),
    #                     "activity_type_id": self.env.ref("bi_pdc.customer_vendor_pdc_not").id,
    #                 }
    ...
    # payment_approval = fields.Many2one('payment.team', string='Payment Approval',default=_get_team_id)
    ...
    #     if next_line:
    #         if not self._can_user_approve(next_line.role_id):
    ...
    #             self.state = 'pending'
    #         elif len(approved_lines) == 2:
    #             self.state = 'authorized'
    #         elif len(approved_lines) == len(self.approval_route_ids):
    #             self.state = 'approved'
    #     else:
