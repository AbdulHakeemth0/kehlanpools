...
class ReportStatementCommon(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for partner_id in partner_ids:
            amount_residual = sum(self.env['account.move.line'].search([('partner_id', '=', partner_id),('account_id.account_type','=',account_type),('date','<',date_start)]).mapped('amount_residual'))
            balance = sum(self.env['account.move.line'].search([('partner_id', '=', partner_id),('account_id.account_type','=',account_type),('date','<',date_start)]).mapped('balance'))
            if amount_residual == 0:
                initial_balance = balance
            else:
                initial_balance = amount_residual   
            res[partner_id] = {
