...
class OutstandingStatement(models.AbstractModel):
    ...
    _name = "report.bi_partner_statement_of_account.outstanding_statement"
    _description = "Partner Outstanding Statement"

    def _display_outstanding_lines_sql_q1(self, partners, date_start,date_end, account_type):
        ...
        return str(
            ...
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name, l.lpo_no, m.lpo_no,
                CASE WHEN l.ref IS NOT NULL
                    THEN l.ref
                    ELSE m.ref
                END,
    ...
    def _display_outstanding_lines_sql_q2(self, sub):
        return str(
            self._cr.mogrify(
                f"""
                SELECT {sub}.partner_id, {sub}.currency_id, {sub}.move_id, {sub}.lpo_no,
    ...
    def _display_outstanding_lines_sql_q3(self, sub, company_id):
        return str(
            ...
            SELECT {sub}.partner_id, {sub}.move_id, {sub}.date,
                {sub}.date_maturity, {sub}.lpo_no, {sub}.name, {sub}.ref, {sub}.debit,
    ...
    def _get_account_display_lines(
        ...
        q1_sql = self._display_outstanding_lines_sql_q1(partners, date_start, date_end, account_type)
        q2_sql = self._display_outstanding_lines_sql_q2("Q1")
        q3_sql = self._display_outstanding_lines_sql_q3("Q2", company_id)
        self.env.cr.execute(
            """
        WITH Q1 as ({}),
             Q2 AS ({}),
             Q3 AS ({})
        SELECT partner_id, currency_id, move_id, date, date_maturity, debit, lpo_no,
